...
Client.prototype.signedUrl = function(bucketName, filename, expiration, amz){
  var epoch = Math.floor(expiration.getTime()/1000);
  var signature = auth.signQuery({
    secret: this.secret,
    date: epoch,
    resource: "/" + bucketName + url.parse(filename).pathname
  });

  if (!amz) {
    return this.url(join(bucketName, filename)) + 
      '?Expires=' + epoch +
